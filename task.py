...
# 1. 페이지 설정
st.set_page_config(
    page_title="보건 체크 및 도우미",
    page_icon="🏥",
    layout="centered"
)

# 2. 스타일 정의 (기존 스타일 유지 및 이전 버튼 스타일 추가)
st.markdown("""
    <style>
...
# 3. 텍스트 데이터베이스 고도화 (의학 정보 팩트 체크 및 항목 추가)
GUIDE = {
    "1. 머리가 아파요 (두통)": """
...
• <b>드레싱:</b> 소독 후 항균 연고를 바르고 습윤 밴드나 거즈로 덮어 상처를 보호합니다.<br>
...
• <span class='urgent'>🚨 알레르기 주의:</span> 복용 후 피부에 붉은 반점, 두드러기, 가려움증이 생기거나 호흡 곤란이 오면 즉시 복용을 중단해야 합니다. 극히 드물게 심각한 피부 박리 증상(스티븐스-존슨 증후군)이 나타날 수 있습니다.<br>
...
• <span class='highlight'>⚖️ 최대 복용량:</span> 이부프로펜 기준 일반적인 <b>하루 최대 복용량은 3,200mg</b>(보통 알약 기준 하루 8~12알 이내, 제품별 처방 준수)입니다.<br>
• <span class='urgent'>🚨 알레르기 주의:</span> 과거 다른 소염진통제를 먹고 눈이나 입술이 붓거나 천식 발작(쌕쌕거림), 두드러기가 났던 경험(NSAIDs 과민반응)이 있다면 이 계열의 약물 복용을 금해야 합니다.<br>
• <span class='urgent'>⚠️ 주의사항:</span> 평소 위궤양이 있거나 신장 기능이 떨어진 분들은 증상을 악화시킬 수 있으니 장기 복용을 피하세요.
""",
...
• <span class='highlight'>⚖️ 최대 복용량:</span> 제품 뒷면의 정해진 용법(예: 하루 3회, 1회 1병/1포)을 엄격히 따라야 합니다. 종합감기약 안에는 이미 아세트아미노펜이 들어있는 경우가 많습니다.<br>
• <span class='urgent'>🚨 알레르기 및 성분 중복 주의:</span> 종합감기약을 먹으면서 타이레놀을 별도로 또 먹으면 <b>아세트아미노펜 과다 복용</b>으로 간 독성이 생길 수 있습니다. 약물 알레르기 유발 항히스타민제 성분을 확인하세요.<br>
• <span class='urgent'>⚠️ 주의사항:</span> 콧물을 멈추는 항히스타민제 성분이 포함되어 있어 <b>심한 졸음</b>을 유발합니다. 운전이나 중요한 공부, 작업 전에 복용을 삼가세요.
""",
...
• <span class='highlight'>🧪 고체(알약) vs 액상(드링크) 소화제 차이점:</span><br>
  - <b>액상 소화제 (생약 성분):</b> 고추, 육계, 건강 등 위장을 자극하는 생약이 들어있어 위장의 운동 자체를 촉진하고 가스를 빠르게 배출하여 속을 시원하게 해줍니다.<br>
  - <b>고체 소화제 (소화효소제):</b> 판크레아틴, 디아스타제 등 탄수화물, 단백질, 지방을 직접 분해하는 효소가 들어있어 음식물 자체를 부수는 데 탁월합니다.<br>
  - <i>따라서 탄수화물/고기를 많이 먹어 속이 꽉 막혔을 때는 알약을, 가스가 차고 더부룩하여 위장을 움직이고 싶을 때는 액상제를 선택하거나 두 종류를 함께 복용하는 것이 효과적입니다.</i><br>
• <span class='highlight'>⚖️ 최대 복용량:</span> 소화효소제는 보통 1회 1~2정씩 하루 3회 식후에 복용하며, 매일 장기 복용 시 몸의 자체 소화 능력이 떨어질 수 있으므로 주의합니다.<br>
• <span class='urgent'>🚨 알레르기 주의:</span> 고체 소화제 중에는 '돼지 췌장(판크레아틴)'에서 유래한 성분이 많으므로, 돼지고기 관련 심한 알레르기가 있는 사람은 성분을 확인해야 합니다.
""",
...
• <span class='highlight'>⚖️ 최대 복용량:</span> 하루 성인 기준 최대 4,000mg (500mg 기준 하루 8알). 편의점용 한 팩이 딱 하루 최대치 분량입니다.<br>
• <span class='urgent'>🚨 알레르기 및 주의:</span> 복용 후 가려움, 호흡 곤란 등 아나필락시스 및 과민 반응이 있으면 즉시 중단하십시오. 음주 후 복용 절대 금지.<br>
• <b>팁:</b> 편의점 안전상비의약품은 약국이 문을 닫은 밤이나 주말에 사용하는 1~2회 응급용입니다. 증상이 지속되면 약국/병원을 찾으세요.
""",
        "img": "https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?w=500&auto=format&fit=crop&q=60"
    },
    "🤢 소화제": {
...
• <span class='highlight'>🧪 형태별 차이:</span> 훼스탈/베아제는 탄수화물과 단백질을 쪼개는 '소화효소제(알약)'이며, 가스활명수-에이는 위장 운동을 돕는 '생약 드링크(액상)'입니다. 증상에 맞춰 선택하세요.<br>
• <span class='urgent'>🚨 알레르기 주의:</span> 정제 소화제에는 돼지 췌장 추출물(소화효소)이 포함되어 있으므로 관련 알레르기 유병자는 주의가 필요합니다.
""",
        "img": "https://images.unsplash.com/photo-1471864190281-a93a3070b6de?w=500&auto=format&fit=crop&q=60"
    },
    "🤧 감기약": {
...
• <span class='urgent'>🚨 중복 및 알레르기 주의:</span> 판콜에이와 판피린티 모두 해열진통 성분(아세트아미노펜)이 이미 다량 포함되어 있습니다. <b>타이레놀과 동시에 교차 복용하면 간에 큰 무리</b>가 가므로 절대 같이 드시지 마세요.<br>
• <b>주의사항:</b> 졸음을 유발하므로 야간 운전이나 위험한 기계 조작 전에는 피하십시오.
""",
        "img": "https://images.unsplash.com/photo-1550572017-edd951b55104?w=500&auto=format&fit=crop&q=60"
    },
}

# 4. 약국 지도 데이터 (퇴계원 지역 가상/실제 기준 위경도 좌표 설정)
PHARMACY_DATA = [
...
# 하단에 공통으로 쓰일 내비게이션 버튼 (기존 기능 유지 + 이전으로 추가)
...
# [2] 우리 집 상비약 백과
    ...
    if st.session_state.medicine is None:
        ...
        # 이미지 추가 (적당한 크기로 조절)
        st.image(MEDICINE[st.session_state.medicine]["img"], width=250, caption="의약품 예시 이미지")
...
# [4] 편의점 상비약 안내
    ...
    if st.session_state.convenience is None:
        ...
        st.image(CONVENIENCE[st.session_state.convenience]["img"], width=250, caption="의약품 예시 이미지")
        st.markdown(f"<div class='result-box'>{CONVENIENCE[st.session_state.convenience]['text']}</div>", unsafe_allow_html=True)
        show_navigation_buttons("convenience")        box-shadow: 0 2px 12px rgba(0,0,0,0.08);
        line-height: 2;
    }
    .urgent { color: #cc0000; font-weight: bold; }
    </style>
